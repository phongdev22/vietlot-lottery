...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'pick_655':
        ...
        await query.message.reply_text(f"🎰 Bộ số 6/55 cho Đại ca: {' - '.join(map(str, nums))}", quote=False)
    
    elif query.data == 'pick_645':
        nums = get_ai_lucky_numbers("6/45", "balanced")
        await query.message.reply_text(f"🎰 Bộ số 6/45 cho Đại ca: {' - '.join(map(str, nums))}", quote=False)
        ...
        await query.message.reply_html(msg, quote=False)
        ...
        await query.message.reply_text(f"🤖 AI Soi Cầu (Gợi ý VIP): {' - '.join(map(str, nums))}", quote=False)
...
async def manual_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args
    if len(args) != 7:
        await update.message.reply_text("❌ Sai cú pháp! Hãy gõ: /buy <loại_vé> <6_số>\nVí dụ: /buy 6/55 01 05 12 24 35 45", quote=False)
        return

    game_type = args[0]
    if game_type not in ["6/45", "6/55"]:
        await update.message.reply_text("❌ Loại vé phải là 6/45 hoặc 6/55", quote=False)
        return

    try:
        nums = [int(x) for x in args[1:]]
    except ValueError:
        await update.message.reply_text("❌ Các số phải là số nguyên!", quote=False)
        return
        
    for n in nums:
        if n < 1 or (game_type == "6/45" and n > 45) or (game_type == "6/55" and n > 55):
            await update.message.reply_text(f"❌ Số không hợp lệ! (1-{'45' if game_type == '6/45' else '55'})", quote=False)
            return
            
    if len(set(nums)) != 6:
        await update.message.reply_text("❌ 6 số không được trùng nhau!", quote=False)
        return
    ...
    if played_today >= limit:
        await update.message.reply_text(f"🚫 Đại ca chơi {played_today} vé rồi, đạt giới hạn {limit} vé.", quote=False)
        return
